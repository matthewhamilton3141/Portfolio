  ...
  return (
    <section id="landing" className="min-h-screen md:h-screen scroll-snap-start grid grid-cols-1 md:grid-cols-2 bg-warm-white transition-colors duration-500 overflow-hidden relative">
      ...
      <div className="landing-left flex flex-col justify-center px-[8vw] py-[100px] md:px-[6vw] md:py-0 md:pl-[12vw] relative z-[1]">
        ...
        >
          <div className="relative w-full block min-h-[55px] sm:min-h-[65px] md:min-h-[110px] pointer-events-auto">
            ...
            >
              <span className="inline whitespace-pre-wrap">{typedText}</span>
              {showCursor && (
                <span className="inline-block w-[4px] h-[0.85em] bg-accent ml-1 align-middle rounded-sm animate-blink" />
              )}
            </div>
        ...
        <p
          className={`font-light text-[clamp(11px,1.8vw,18px)] text-ink-soft mt-2 transition-all duration-800 ${
        ...
        >
          <img 
            ...
            alt="" 
            className="h-[120px] w-auto dark:invert dark:opacity-80"
      ...
      <div className="landing-right relative flex flex-col justify-center px-[8vw] py-10 md:px-[5vw] md:py-[80px] md:pr-[8vw] bg-cream transition-colors duration-500 overflow-y-auto overflow-x-hidden">
        ...
        <div className="flex flex-row items-stretch gap-6 mb-6 w-full max-w-[550px]">
          ...
          <div
            className={`w-[180px] md:w-[200px] aspect-[3/4] rounded-sm relative overflow-hidden flex-shrink-0 transition-all duration-800 ${
          ...
          <div 
            className={`w-[180px] md:w-[200px] max-w-[200px] aspect-[3/4] rounded-sm border border-dashed border-border/60 bg-surface/20 relative overflow-hidden transition-all duration-800 delay-300 hidden sm:block ${
              showElements.photo ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"
            }`}
          >
            <LogoSandbox logos={["/images/waterloologo.png", "/images/nvidialogo.png", "/images/jamhackslogo.png", "/images/bayernlogo.png", "/images/overwatchlogo.png", "/images/riotlogo.png"]} />
          </div>
        </div>

        <div
          className={`about-bio transition-all duration-800 ${
...
export const LogoSandbox = React.memo(function LogoSandbox({ logos }: { logos: string[] }) {
  ...
  useEffect(() => {
    ...
    let targetSize = 84
    if (logos.length > 3) targetSize = 64
